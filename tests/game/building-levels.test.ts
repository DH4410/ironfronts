...
function context(): SimContext {
  return {
    state: {
      countries: { 1: { phase: 3, stockpile: { funds: 100_000, manpower: 0, food: 0, stone: 100_000, metal: 100_000, oil: 0 } } },
      provinceOwners: { 10: 1 },
...
describe('five-level building framework', () => {
  it('gives every building five increasingly expensive and slower levels', () => {
    for (const building of Object.values(BUILDINGS)) {
      expect(building.tiers).toHaveLength(5);
      for (let i = 1; i < 5; i += 1) {
  ...
  it('accelerates unit work from 1x at level I to 3x at level V', () => {
    const ctx = context();
    expect(unitProductionWorkRate(ctx, 10, 'infantry')).toBe(UNIT_PRODUCTION_RATE_BY_LEVEL[1]);
    ctx.state.provinceBuildings[10].barracks = 5;
    expect(unitProductionWorkRate(ctx, 10, 'infantry')).toBe(3);
  });

  it('scales rich resource sites through level V', () => {
    expect(maximumResourceTier('oilPump', 0.979)).toBe(4);
    expect(maximumResourceTier('oilPump', 0.98)).toBe(5);
    expect(RESOURCE_TIER_PASSIVE[5]).toBeGreaterThan(RESOURCE_TIER_PASSIVE[4]);
    expect(RESOURCE_TIER_ENGINEER_CAP[5]).toBeGreaterThan(RESOURCE_TIER_ENGINEER_CAP[4]);
    expect(RESOURCE_TIER_ENGINEER_MULTIPLIER[5]).toBeGreaterThan(RESOURCE_TIER_ENGINEER_MULTIPLIER[4]);
  });
});
