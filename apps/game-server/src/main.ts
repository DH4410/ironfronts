...
// to fast-forward the simulation (movement/production/combat) without touching
// any balance constant.
const simTickHours = SIMULATION_TICK_HOURS * config.devSimSpeed;
if (config.devSimSpeed !== 1) log('warn', 'dev_sim_speed_active', { multiplier: config.devSimSpeed });
const simulationTimer = setInterval(() => runtime.tick(simTickHours), SIMULATION_INTERVAL_MS);
