...
export function buildBankField(provinceIds, width, height, worldWidth, worldHeight, landOverride) {
  ...
  for (let index = 0; index < land.length; index += 1) {
    ...
    const proximity = 1 - clamp(Math.abs(safeDistance) / bankRange, 0, 1);
  ...
  return {
    field,
    report: { method: 'topology-preserving-smoothed-signed-distance', smoothingPasses: 2, texelSize, edgeHalfWidth, bankRange, bankPixels },
  };
}
