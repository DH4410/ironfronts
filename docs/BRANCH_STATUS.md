...
| `feat/graphics-quality-settings` | Graphics Quality presets. | **#43 (OPEN, `MERGEABLE`)** as of 2026-08-28 | `7ff1e2e` | `src/graphics/quality.ts`, `src/chunk-visibility.ts`, `src/renderer.ts`, `src/main.ts`, `src/menu/menu.ts`, `src/menu/menu.css`, `index.html`, `tests/graphics-quality.test.ts`. | Active, PR open. | **No — open PR.** |
...
| `origin/feat/in-game-command-ui-v2` | **Development / integration only.** `review/claude-ui-stack` + PR #43 (3 conflicts resolved, see `PR_MERGE_PLAN.md`) + the new in-game command UI v2 (`src/ui/*`, typed `StrategicUiState`, event-driven HUD, shader selected-province outline, debug gating). Pushed to `DH4410` so it is safe/previewable. | **none — do NOT open upstream yet** (Zoande has not merged #29–#43). | `origin/review/claude-ui-stack` | `src/ui/ui-state.ts`, `src/ui/game-ui.ts`, `src/ui/game-ui.css`, `src/ui/army.ts`; edits to `src/main.ts`, `src/renderer.ts`, `src/shaders/lines.ts`; `docs/*`. | Active. | **No.** After the batch merges, rebase a clean branch off new `main` and open a focused UI-only PR (steps in the final report). Not force-pushed. |
