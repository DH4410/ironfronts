...
Clients send authenticated command envelopes with a unique command ID. Commands cover move, attack, retreat, split, stop, extract, produce, build, and rally. The server validates ownership and all game rules. A successful command is published before its acknowledgement; the acknowledgement carries the applied projection revision. Duplicate command IDs return the cached acknowledgement.
...
Nested server messages are validated with concrete schemas, including projected armies, queues, capabilities, timeline data, catalogs, world hashes, deltas, and kind-specific events. Events have monotonic IDs and required ownership/location fields where applicable. The publisher keeps a bounded per-country backlog until every current recipient accepts delivery. Clients deduplicate IDs.

The gateway rejects payloads above 32 KiB and closes a connection with code 1013 once queued socket data exceeds 2 MB. Debug clock/speed messages are ignored unless the server explicitly enables development controls.
