/**
 * Scenario / menu-selection types (§2, §4).
 *
 ...
 * data — the scenario never bakes pixel rectangles into the renderer (§4).
 */
...
export interface ScenarioDef {
  ...
  /** Display date; also the campaign clock's start date (§2). */
  readonly startDate: string;
  readonly mode: GameMode;
  /**
   * Playable countries. `'all'` = every country with territory (World at War,
   * Sandbox). Otherwise a curated, scenario-specific faction list by name (§3),
  ...
  /** Campaign requires a working economy to build; sandbox does not (§4). */
  readonly economyEnabled: boolean;
  readonly blurb: string;
}

/** The typed hand-off from menu to game (§2). */
